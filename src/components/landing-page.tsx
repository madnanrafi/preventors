import { Phone } from "lucide-react";
import { Hero } from "./hero";
import { Benefits } from "./benefits";
import { SocialProof } from "./social-proof";
import { FAQ } from "./faq";
import { CTASection } from "./cta-section";

function WhatsAppIcon(props: React.SVGProps<SVGSVGElement>) {
    return (
        <svg viewBox="0 0 24 24" fill="currentColor" {...props}>
            <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.008-.57-.008-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413Z" />
        </svg>
    );
}

export function LandingPage() {
    return (
        <div className="flex flex-col w-full font-sans text-gray-900">
            {/* Header / Navigation */}
            <header className="sticky top-0 z-50 w-full border-b bg-white/95 backdrop-blur supports-[backdrop-filter]:bg-white/60">
                <div className="container mx-auto px-4 md:px-6 h-16 flex items-center justify-between">
                    <div className="flex flex-col">
                        <span className="text-xl font-bold text-blue-600 leading-none">Preventors</span>
                        <span className="text-xs font-semibold text-slate-600 tracking-wide">Pest Control Services</span>
                    </div>
                    <div className="flex items-center gap-3">
                        <a
                            href="https://wa.me/923375627617"
                            target="_blank"
                            rel="noopener noreferrer"
                            className="flex items-center gap-2 text-sm font-medium text-slate-700 hover:text-green-600 transition-colors"
                            aria-label="Contact on WhatsApp"
                        >
                            <div className="flex items-center justify-center w-9 h-9 rounded-full bg-green-50 text-green-500 hover:bg-green-100 transition-colors">
                                <WhatsAppIcon className="w-5 h-5" />
                            </div>
                            <span className="hidden lg:inline-block">WhatsApp</span>
                        </a>

                        <a
                            href="tel:03375627617"
                            className="flex items-center gap-2 text-sm font-medium text-slate-700 hover:text-blue-600 transition-colors"
                        >
                            <div className="flex items-center justify-center w-9 h-9 rounded-full bg-blue-50 text-blue-600 hover:bg-blue-100 transition-colors">
                                <Phone className="w-4 h-4" />
                            </div>
                            <span className="hidden sm:inline-block">0337-5627617</span>
                        </a>
                    </div>
                </div>
            </header>

            <Hero />
            <Benefits />
            <SocialProof />
            <FAQ />
            <CTASection />
            {/* Footer can be simple enough to be inline or separate, checking plan */}
            <footer className="py-6 text-center text-sm text-gray-500 border-t bg-slate-50">
                <p>© {new Date().getFullYear()} Preventors Pest Control Services. All rights reserved.</p>
                <p className="mt-1">Rawalpindi, Punjab, Pakistan</p>
            </footer>
        </div>
    );
}
